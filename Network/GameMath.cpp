...
const float4 float4::FORWARD = { 0.0f, 0.0f, 1.0f };
...
float4 float4::QuaternionMultiplyfloat4x4(const float4x4& _Value) const
{
	float4 Q = float4::MatrixToQuaternion(_Value);

	return DirectX::XMQuaternionMultiply(DirectVector, Q.DirectVector);
}
